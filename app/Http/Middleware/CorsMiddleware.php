...
class CorsMiddleware
{
    ...
    protected function setCorsHeaders($req, $rsp) {
        ...
        if ($req->isMethod('OPTIONS')) {
            ...
            $responseType = substr(get_class($rsp), strrpos(get_class($rsp), '\\') + 1);

            if ($responseType !== 'BinaryFileResponse') {
                $this->setOrigin($req, $rsp);
                $this->setExposeHeaders($req, $rsp);
                $this->setAllowCredentials($req, $rsp);
            } else {
                $rsp->headers->set('Access-Control-Allow-Origin' , '*');
                $rsp->headers->set('Access-Control-Allow-Methods', 'POST, GET, OPTIONS, PUT, DELETE');
                $rsp->headers->set('Access-Control-Allow-Headers', 'Content-Type, Accept, Authorization, X-Requested-With, Application');
            }
